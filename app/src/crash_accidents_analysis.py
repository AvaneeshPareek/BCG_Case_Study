...
class USVehicleAccidentAnalysis:
    ...
    def __init__(self, spark, config):
        ...
        self.df_charges = load_csv_data_to_df(spark, input_file_paths["Charges"])
        self.df_damages = load_csv_data_to_df(spark, input_file_paths["Damages"])
        self.df_endorse = load_csv_data_to_df(spark, input_file_paths["Endorse"])
        self.df_primary_person = load_csv_data_to_df(spark, input_file_paths["Primary_Person"])
        self.df_units = load_csv_data_to_df(spark, input_file_paths["Units"])
        self.df_restrict = load_csv_data_to_df(spark, input_file_paths["Restrict"])
    ...
    def count_2_wheeler_accidents(self, output_path, output_format):
        ...
        """
        df = self.df_units.select('CRASH_ID','VEH_BODY_STYL_ID')\
    ...
    def top_5_vehicle_makes_for_fatal_crashes_without_airbags(
        ...
        """
        df = self.df_units.join(self.df_primary_person,on = ['CRASH_ID','UNIT_NBR'],how = 'inner')\
        .select(self.df_units.CRASH_ID,self.df_units.UNIT_NBR,self.df_units.VEH_MAKE_ID,
        self.df_primary_person.PRSN_TYPE_ID,self.df_primary_person.PRSN_INJRY_SEV_ID,self.df_primary_person.PRSN_AIRBAG_ID)\
